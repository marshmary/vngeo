...
const ProvinceDebugger: React.FC = () => {
  ...
  return (
    <div className="fixed top-4 left-4 bg-card p-4 rounded-card shadow-overlay max-w-sm max-h-96 overflow-y-auto z-50">
      ...
      <div className="text-xs space-y-2 mb-3">
        {Object.entries(ZONE_PROVINCES).map(([zoneId, zoneProvinces]) => (
          <div key={zoneId} className="border border-border rounded p-2">
      ...
      <details className="text-xs">
        ...
        <div className="mt-2 space-y-1">
          {provinces.map(province => (
            <div key={province} className="border-b border-border pb-1">
              {province}
            </div>
          ))}
        </div>
